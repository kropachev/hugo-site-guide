...
- `mainSections = ["posts", "projects"]` - указываем секции контента, с которых начинаем. Это стандартный параметр Hugo (не параметр темы), поэтому он задаётся на верхнем уровне конфига, а не под `[params]`. Сначала постов нет - сайт живет одной страницей "Обо мне".
